...
filename = 'Lidar_IMU_Parallel_frequency.csv'
datafile = 'Lidar_IMU_Parallel_data.csv'
pool = Pool()
timeDiffer = []
rowList = []
def getIMU(q):
...
##            Lidardata = result2.get(timeout=1)
            ...
            row = [currentTime,Lidardata,IMUdata] #the row being written to csv file, just x and y accel
            rowList.append(row)
            print(row)
